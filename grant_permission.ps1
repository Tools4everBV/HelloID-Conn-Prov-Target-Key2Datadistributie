...
if(-Not($dryRun -eq $True)) {
    try{
	    ...
	    $OracleQuery = "
            MERGE INTO DDS_FUNCTIE_AUTORISATIE t1
				  USING
				  	(SELECT DISTINCT '$aRef' AS GEBRUIKERNR, '$($pRef.Id)'AS FUNCTIECODE
					 FROM DDS_FUNCTIE_AUTORISATIE) t2
				  ON (t1.GEBRUIKERNR = t2.GEBRUIKERNR AND t1.FUNCTIECODE = t2.FUNCTIECODE)
			  WHEN NOT MATCHED THEN
			  	INSERT VALUES (t2.GEBRUIKERNR, t2.FUNCTIECODE)"
